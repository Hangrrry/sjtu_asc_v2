...
def coordinate_change(height=20, pos_=[0, 0, 20], yaw=0):
    ...
    # 通过ros获取飞行的高度以及X与Y的值 end_time - start_time > 180 此处默认设置为20
    X0 = pos_[0]
    Y0 = pos_[1]
    Z0 = pos_[2]
...
def coordinate_change2(height=25, pos_=[0, 0, 25], yaw=0,cropTensorList=[[0, 0], [0, 0], [0, 0], [0, 0]]):
    ...
    world = rotate_around_x() @ rotate_around_z(yaw=np.deg2rad(yaw)) @ camera
...
if __name__=="__main__":
    ...
    while not rospy.is_shutdown():
        ...
        if cap.isOpened():
            ...
            if success == True:
                if (end_time - start_time > 180 or stop_flag.data == 666.666):
                    break

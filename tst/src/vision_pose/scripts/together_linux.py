...
def coordinate_change2(height=25, pos_=[0, 0, 25], yaw=0.00000000,cropTensorList=[[0, 0], [0, 0], [0, 0], [0, 0]]):
    ...
    # height为相机到地面高度值
    camera = mtx_ @ target * height  # @表示矩阵相乘，*表示矩阵乘以一常数
    # 最好写进问价里面好后续分析
    # print("camera",camera)
    # print("rotate_z",rotate_around_z(yaw=np.deg2rad(yaw)))
    # print("rotate_x",rotate_around_x())
    world = rotate_around_x() @ rotate_around_z(yaw=yaw - pi / 2) @ camera
    result = [world[0] + X0, world[1] + Y0, world[2] + Z0]
...
if __name__=="__main__":
    ...
    while not rospy.is_shutdown():
        ...
        if cap.isOpened():
            ...
            if success == True:
                if (end_time - start_time > 120 or stop_flag.data == 666.666):
                    break
    ...
    for k in range(0, object_sum):  # 识别到数字的有效
        ...
        if middle_num == int(dataList[k].get_num()):
            zero_pose[0] += coordinate_change2(height= 25, pos_ = [dataList[k].get_pos()[0],dataList[k].get_pos()[1],25], yaw = dataList[k].get_yaw(),cropTensorList=dataList[k].get_cropTensorList())[0]#coordinate_change2(height= dataList[k].get_pos()[2], pos_ = dataList[k].get_pos(), yaw = dataList[k].get_yaw())[0]
            zero_pose[1] += coordinate_change2(height= 25, pos_ = [dataList[k].get_pos()[0],dataList[k].get_pos()[1],25], yaw = dataList[k].get_yaw(),cropTensorList=dataList[k].get_cropTensorList())[1]#coordinate_change2(height= dataList[k].get_pos()[2], pos_ = dataList[k].get_pos(), yaw = dataList[k].get_yaw())[1]
            zero_pose[2] += coordinate_change2(height= 25, pos_ = [dataList[k].get_pos()[0],dataList[k].get_pos()[1],25], yaw = dataList[k].get_yaw(),cropTensorList=dataList[k].get_cropTensorList())[2]#coordinate_change2(height= dataList[k].get_pos()[2], pos_ = dataList[k].get_pos(), yaw = dataList[k].get_yaw())[2]
            middle_ += 1
            #print("middle_ += 1")
